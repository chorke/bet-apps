 ...
 * @author Chorke
 */
public class ClearFavoritStatisticMiner implements StatisticMiner{
    ...
    public double getResultByBetingOnHome(Match match){
        double bet1 = match.getBet1();
        if(match.getWinner() == 1 && bet1 != match.getBet2()){
            return bet1;
        } else {
            return 0.0;
        }
    ...
    public double getResultByBetingOnGuest(Match match){
        double bet2 = match.getBet2();
        if(match.getWinner() == 2 && match.getBet1() != bet2){
            return bet2;
        } else {
            return 0.0;
        }
    ...
    public double getResultByBetingOnTie(Match match){
        if(match.getWinner() == 0 && match.getBet1() != match.getBet2()){
